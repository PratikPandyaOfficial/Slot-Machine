...
struct ShadowModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            .shadow(color: .colorTrasparentBlack, radius: 0, x: 0, y: 6)
    }
}
...
struct ScoreNumberModifier: ViewModifier{
    func body(content: Content) -> some View {
        content
            .shadow(color: .colorTrasparentBlack, radius: 0, x: 0, y: 3)
            .layoutPriority(1)
    }
}
...
struct ScoreContainerModifier: ViewModifier{
    func body(content: Content) -> some View {
        content
            ...
            .background(
                Capsule()
                    .foregroundStyle(.colorTrasparentBlack)
            )
    }
}
...
struct CasinoChipsModifier: ViewModifier{
    func body(content: Content) -> some View {
        content
            .scaledToFit()
            .frame(height: 64)
            .animation(.default, value: /*@START_MENU_TOKEN@*//*@PLACEHOLDER=Value@*/0/*@END_MENU_TOKEN@*/)
            .modifier(ShadowModifier())
    }
}
